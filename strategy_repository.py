    ...
    def _load_strategies_from_disk(self):
        ...
        for strategy_name, strategy_data in strategies_data.items():
            try:
                ...
                logger.debug(f"🔥 PERSISTENCE: loaded strategy {strategy.name} with {len(strategy.api_calls)} API calls")
    ...
    def delete_strategy(self, name: str) -> bool:
        ...
        with self._storage_lock:
            if normalized_name in self.strategies:
                ...
                logger.info(f"🔥 STRATEGY DELETED: name={strategy.name} display_name={getattr(strategy, 'display_name', strategy.name)}")
                return True
            return False
