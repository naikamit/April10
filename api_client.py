...
class SignalStackClient:
    ...
    async def _make_request(self, payload: Dict[str, Any], strategy: Strategy) -> Tuple[bool, Dict[str, Any]]:
        ...
        """
        logger.info(f"🔥 API REQUEST: strategy={strategy.name} payload={payload}")
        ...
        while retry_count <= self.max_retries:
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    logger.info(f"🔥 API RESPONSE: strategy={strategy.name} response={response_data}")
                    ...
                    if 'status' in response_data:
                        if response_data['status'] in ['filled', 'accepted']:
                            ...
                            logger.error(f"🔥 ERROR: strategy={strategy.name} validation_error={response_data.get('message', 'Unknown error')}")
                            return False, response_data
                    
                    # If we get here, the response was not successful but also not a validation error
                    logger.warning(f"🔥 ERROR: strategy={strategy.name} unexpected_response={response_data}")
                    retry_count += 1
                    
            except (httpx.TimeoutException, httpx.ConnectError) as e:
                logger.error(f"🔥 ERROR: strategy={strategy.name} api_timeout={self.timeout}s retrying_attempt={retry_count+1}/{self.max_retries+1} error={str(e)}")
                retry_count += 1
            except Exception as e:
                logger.exception(f"🔥 ERROR: strategy={strategy.name} unexpected_api_error={str(e)}")
                retry_count += 1
                
            if retry_count <= self.max_retries:
                logger.info(f"🔥 API RETRY: strategy={strategy.name} retrying_in={self.retry_delay}s")
                await asyncio.sleep(self.retry_delay)
            else:
                logger.error(f"🔥 ERROR: strategy={strategy.name} max_retries_exceeded={self.max_retries}")
    ...
    async def buy_symbol(self, symbol: str, quantity: int, strategy: Strategy) -> Tuple[bool, Optional[float], Dict[str, Any]]:
        ...
        logger.info(f"🔥 BUYING SHARES: strategy={strategy.name} symbol={symbol} quantity={quantity}")
        
        success, response = await self._make_request(payload, strategy)
        
        if success and 'price' in response:
            logger.info(f"🔥 API RESPONSE: strategy={strategy.name} action=buy symbol={symbol} price={response.get('price')} quantity={quantity}")
    ...
    async def close_position(self, symbol: str, strategy: Strategy) -> Tuple[bool, Optional[float], Optional[int], Dict[str, Any]]:
        ...
        if success and response.get('status') == 'filled':
            logger.info(f"🔥 API RESPONSE: strategy={strategy.name} action=close symbol={symbol} price={response.get('price')} quantity={response.get('quantity')}")
        ...
        if success and response.get('status') == 'accepted':
            logger.info(f"🔥 API RESPONSE: strategy={strategy.name} action=close symbol={symbol} no_positions_to_close")
